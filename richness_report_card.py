...
mean_rate_genus = 0.000028
std_dev_genus = 0.00001
...
def compute_genus_per_acre(df, waterbody_area_ac):
    """
    Computes the number of different genus per acre per year for each waterbody.
    """
    # Count unique genus per year and waterbody
    genus_counts = df.groupby(['year', 'waterbody'])['genus'].nunique().unstack(fill_value=0)

    # Normalize by waterbody acreage
    genus_per_acre = genus_counts.div(pd.Series(waterbody_area_ac), axis=1)
    return genus_per_acre

def assign_ryg_genus(genus_per_acre, mean_rate, std_dev):
    """
    Assigns R, Y, or G based on the genus per acre per year.
    """
    # Define thresholds
    ...
    ryg = genus_per_acre.applymap(categorize)
    return ryg
...
def plot_report_card(ryg):
    ...
    # Add titles
    plt.title("Waterbody Invasive Richness Report Card", fontsize=16)
    plt.suptitle("Based on genus diversity per acre per year", fontsize=12, y=0.92)
    plt.tight_layout()

    # Save and show the plot
    plt.savefig("Plots/richness_report_card.png")
    plt.show()
...
def main():
    ...
    # Compute genus per acre per year
    genus_per_acre = compute_genus_per_acre(df_clean, waterbody_area_ac)

    # Assign R/Y/G values
    ryg = assign_ryg_genus(genus_per_acre, mean_rate_genus, std_dev_genus)
